...
func TestTorchTitanInstallGuidanceExplainsPythonResolution(t *testing.T) {
	...
	for _, expected := range []string{"Python 3.11", "hash -r", "python3 -m pip", "--extra-index-url https://pypi.org/simple", "Pin the desired TorchTitan nightly version", "torch.cuda.is_available()", "torch.distributed.is_nccl_available()"} {
